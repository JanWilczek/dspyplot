...
class PlotPeriodCommand:
    ...
    def __call__(self):
        ...
        plt.arrow(
            self.period_start, self.arrow_y, self.period_length, 0, **self.arrow_style
        )
        plt.arrow(
            self.period_end, self.arrow_y, -self.period_length, 0, **self.arrow_style
        )
        plt.text(
            self.label_x, self.label_y, self.label_text, horizontalalignment="center"
        )
...
def stem(points, bin_indices=None, alpha=1.0, color=style.color):
    if bin_indices is not None:
        markerline, stemlines, baseline = plt.stem(
            bin_indices, points, **style.stem_params
        )
    else:
...
def stem_signals_and_save(
    signal1,
    signal2,
    output_path: Path,
    signal1_alpha=1.0,
    signal2_alpha=1.0,
    signal1_color=style.color,
    signal2_color=style.color,
):

    plt.figure(figsize=(12, 6))
    stem_signals(
        signal1, signal2, signal1_alpha, signal2_alpha, signal1_color, signal2_color
    )
...
def plot_analog_signal_and_save(
    x, y, output_path: Path, xlabel, xticks, xtick_labels, ylim, yticks, ytick_labels
):
    plt.figure(figsize=(12, 6))
...
def plot_magnitude_response_and_save(
    b, a, output_path, ylim, yticks=None, db=False, cutoff_frequency=None
):
    ...
    if cutoff_frequency is not None:
        ...
        plt.text(
            cutoff_frequency, ylim[0] - 5.5 if db else -0.18, r"$\omega_c$", ha="center"
        )
...
def plot_phase_response_and_save(
    b, a, output_path, yticks=None, ytick_labels=None, unwrap=False
):
    """b, a have their meaning from scipy.signal, see scipy.signal.butter"""
...
def pole_zero_plot(zeros, poles=None):
    ...
    for zero in zeros:
        zero = complex(zero)
        circle = Circle(
            (zero.real, zero.imag), 0.04, edgecolor="k", fill=False, linewidth=3
        )
        ax.add_patch(circle)
...
def plot_z_plane():
    ...
    offset_axis_ticklabels_by(fig, ax.xaxis, 5 / 72.0, -5 / 72.0)
    offset_axis_ticklabels_by(fig, ax.yaxis, 15 / 72.0, -15 / 72.0)
...
def plot_on_unit_circle_in_3d_and_save(
    two_sided_magnitude_spectrum, output_path, zlim=None, zticks=None
):
    if zticks is None:
        zticks = []
...
def plot_analog_magnitude_responses_in_db_and_save(
    b_array, a_array, output_path, legend, ylim
):
    plt.figure(figsize=(12, 6))
...
def plot_magnitude_responses_and_save(
    b_array,
    a_array,
    sampling_rate,
    output_path,
...
def strip_zeros(string: str):
    return string.rstrip("0").rstrip(".") if "." in string else string


def print_signal(signal):
    stringified = [strip_zeros(f"{signal[i]:.3f}") for i in range(0, len(signal))]
